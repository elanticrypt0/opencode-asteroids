...
class Ship {
  ...
  update(dt) {
    ...
    // Límite de velocidad
    const maxSpeed = this.speedTimer > 0 ? 660 : 330;
    const speed = Math.hypot(this.vx, this.vy);
    if (speed > maxSpeed) {
  ...
  draw() {
    ...
    if (this.thrusting && Math.random() > 0.35) {
      ...
      const len = boosted ? rand(12, 24) : rand(6, 14);
      ctx.beginPath();
      ctx.moveTo(-8, -4);
      ctx.lineTo(-8 - len, 0);
      ctx.lineTo(-8,  4);
      ctx.strokeStyle = boosted ? 'rgba(0, 255, 255, 0.85)' : 'rgba(255, 130, 0, 0.85)';
      ctx.stroke();
    }
...
function drawHUD() {
  ...
  if (ship.speedTimer > 0) {
    ...
    ctx.fillText(`VEL ${ship.speedTimer.toFixed(1)}s`, W / 2, 44);
  }
}
